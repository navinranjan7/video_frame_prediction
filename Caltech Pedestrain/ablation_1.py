...
with tf.device('/gpu:3'):
    ...
    #        frame_rnn1 = self.reshape_rnn_out(frame_rnn1)
            ...
            decoder4 = self.decode_block4(frame5, frame4, frame4, 'main',  optical_feed=False, concatenation=False, training=training)
            decoder3 = self.decode_block3(decoder4, frame3, frame4, 'main',  optical_feed=False, concatenation=False, training=training)
            decoder2 = self.decode_block2(decoder3, frame2, frame4, 'main',  optical_feed=False, concatenation=False, training=training)
            decoder1 = self.decode_block1(decoder2, frame1, frame4, 'main',  optical_feed=False, concatenation=False, training=training)
    ...
    #            loss_value_opt = train_optical(x_frame, x_optical, output, output_optical)
                ...
                if start % 8000 == 0:
                    print(f"Epochs {epoch}  {start} Training Loss: {float(loss_value)}")
    ...
    #                o = cv2.hconcat([out,out1])
                    cv2.imwrite(f"/home/navin/paper_5/dataset/Output/ablation_1_1/{epoch}_{k+index}.png", out)
                    cv2.imwrite(f"/home/navin/paper_5/dataset/Output/ablation_1_2/{epoch}_{k+index}.png", out1)
